...
**The pearson correlation coefficient of 0.636556 indicate positive association between TimeTv and Happiness which means that student spending more time watching TV have greater percentage rate happiness. The r=.636556 indicate a large effect because it is >+.50. The p-value = 0.03521 which is considered  **
...
```
**I have picked TimeTV and happiness while the controlling variable is Gender. I noticed that the partial correlation between TimeTV and Happiness is 0.6311611 which is close to the correlation when the effect of Gender is not controlled (r=0.636556) however the P value when Gender is controlled increases 0.06832112 which mean that is less statistically significant compare to when effect of Gender is not controlled (p-value) is above .05 whole when effect of Gender is not controlled (p-value = 0.03521) which is considered to be statistically significant since it's value is <.05 **